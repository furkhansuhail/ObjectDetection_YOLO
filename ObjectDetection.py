...
class ObjectDetection:
    def __init__(self):
        self.VideoPath = ("C:/AI Models/AllOtherProjects/ComputerVisionPeopleCounter/"
                          "ObjectVideoTracking/sample-videos-master/classroom.mp4")
        self.cap = None
        self.ModelDriver()
    ...
    def VideoProcessing(self):
        ...
        if not ret or frame is None:
            print("❌ Failed to read the video frame. Please check the video path or format.")
            return

        plt.imshow(frame[:, :, ::-1])  # OpenCV uses BGR, matplotlib uses RGB
        plt.title("First Frame")
        plt.axis("off")
        plt.show()

        # Applyting Object Detection Code on first frame
        ObjDetection  = ObjectDetectionPipeline(device='cuda', threshold=0.5)
        plt.figure(figsize=(10, 10))
        plt.imshow(ObjDetection(frame)[:, :, ::-1])
        plt.show()
    ...
    def _plot_boxes(self, output_img, labels, boxes):
        ...
        for label, (x1, y1, x2, y2) in zip(labels, boxes):
            if (x2 - x1) * (y2 - y1) < 0.25:
                ...
                cv2.putText(output_img, self.classes_to_labels[label - 1], (int(x1), int(y1) - 10),
